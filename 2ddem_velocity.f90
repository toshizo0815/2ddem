module constant
    ...
    integer, parameter :: Nstep = 2000000, particle_number = 288, time_step = 1000      !time_stepは描写する時間間隔
    ...
    !粒子のパラメータ
    real(8), parameter :: time = 1000.0d0, d = 1.0d0, m = 1.0d0, inertia = (2.0d0 / 5.0d0) * m * ((d / 2.0d0) ** 2)    !m,g,dで規格化
    ...
    !斜面のパラメータ
    real(8), parameter :: alpha_degree = 6.0d0
...
contains
    ...
    subroutine particle_contact(d1, d2, x1, x2, v1, v2, w1, w2, deltat_prev, deltat, fn, ft, torque, Us)
...
program dem_2d
    ...
    integer :: counter(0:v_cell-1,0:v_cell-1) = 0
    real(8) :: v_ave(0:1,0:v_cell-1,0:v_cell-1) = 0.0d0
    ...
    call makedirs('./vector_data_'//d_char//'_'//alpha_char//'_'//particle_num_char)
    ...
    do step = 1, Nstep 
        if(particle_number > 0) then 
            ...
            do i = 1, particle_number   !注目する粒子
                ...
                fwn(0:1,i), fwt(0:1,i), torque_wall(i), torque_wall2(i), u_wall, Us)
                ...
                energy_sum = energy_sum + (1.0d0/2.0d0) * m * (v(0,i)**2 + v(1,i)**2) + (1/2) * inertia * w(i) ** 2 + &
                m * vec_length(g(0:1), zero(0:1)) * x(1,i) 
                ...
                p(0,i) = x(0,i) + (d / 2) * cos(theta(i))
                p(1,i) = x(1,i) + (d / 2) * sin(theta(i))
        ...
        if(step > step_fixed) then
            ...
            Uall = Uall + (1.0d0/2.0d0) * m_cylinder * vec_length(v_center(0:1,step), zero(0:1))**2 +&
            (1.0d0/2.0d0) * inertia_cylinder * w_wall(step)**2 + m_cylinder * vec_length(g(0:1), zero(0:1)) * x_center(1,step)
        endif

        p_center(0,step) = x_center(0,step) + r_cylinder * cos(theta_center(step))
        p_center(1,step) = x_center(1,step) + r_cylinder * sin(theta_center(step))
        ...
        if (mod(step,10000)==0) then
            ...
            open(30,file='./vector_data_'//d_char//'_'//alpha_char//'_'//particle_num_char//'/v_vector_data_'//step_char)
            do i = 1, particle_number
                v_ave(:,floor(x(0,i) - x_center(0,step))+v_cell/2, floor(x(1,i) - x_center(1,step))+v_cell/2) = &
                v_ave(:,floor(x(0,i) - x_center(0,step))+v_cell/2, floor(x(1,i) - x_center(1,step))+v_cell/2) + v(:,i) - v_center(:,step)
                counter(floor(x(0,i) - x_center(0,step))+v_cell/2, floor(x(1,i) - x_center(1,step))+v_cell/2) = &
                counter(floor(x(0,i) - x_center(0,step))+v_cell/2, floor(x(1,i) - x_center(1,step))+v_cell/2) + 1
            enddo
            do i = 0, v_cell - 1
                do j = 0, v_cell - 1
                    if (counter(i,j) /= 0) then
                        v_ave(:,i,j) = v_ave(:,i,j) / counter(i,j)
                    endif
                    write(30, *) i, j, v_ave(:,i,j)
                enddo
            enddo
            close(30)
            v_ave(:,:,:) = 0.0d0
