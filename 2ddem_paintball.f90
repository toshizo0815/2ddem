module constant
    ...
    integer, parameter :: Nstep = 1000000, particle_number = 1440, time_step = 1000      !time_stepは描写する時間間隔
...
contains
    ...
    subroutine mkvtk(step, dirname, x, theta, color)    !dirnameというフォルダ内にstep目のvtkファイルを作成
        integer, intent(in) :: step
        real(8), intent(in) :: x(0:1,1:array_size), theta(1:array_size), color(1:particle_number)
        ...
        !色の指定
        write(10,'(a)') 'SCALARS color float'
        write(10,'(a)') 'LOOKUP_TABLE default'
        do i = 1, particle_number
            write(10,*) color(i)
        enddo
...
contains
    ...
    subroutine particle_contact(d1, d2, x1, x2, v1, v2, w1, w2, deltat_prev, deltat, fn, ft, torque)
...
program dem_2d
    ...
    real(8) :: color(1:particle_number) = 1.0d0
...
!初期状態を設定 {
    ...
    call mkvtk(0, './vtk_data', x_prev(:,:), theta_prev(:), color(:))

    do step = 1, Nstep 

        if (step == step_fixed) then 
            do i = 1, particle_number
                if (0.9d0*(r_cylinder+deltad) < x_prev(0,i) .and. x_prev(0,i) < 1.1d0*(r_cylinder+deltad)) then
                    color(i) = 2.0d0
                endif
            enddo
        endif 
        ...
        if(particle_number > 0) then 
            ...
            do i = 1, particle_number   !注目する粒子
                ...
                fwn(0:1,i), fwt(0:1,i), torque_wall(i), torque_wall2(i), u_wall)
            ...
            if (mod(step, vtk_step)==0) then
                call mkvtk(step, './vtk_data', x(:,:), theta(:), color(:))
            endif
        endif 
