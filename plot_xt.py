...
#---データの読み込み---
state='2_09_040'    #---粒子径と斜面角度---
x, y=np.loadtxt('xt_'+state, unpack=True)
x_th, y_th=np.loadtxt('xt_rigid', unpack=True)
y_lin=x
...
ax.plot(x[0], y[0], color="#000000", linestyle='solid', linewidth=2, marker='o', markersize=2, label=r"$simulation$")    #ラベルを一回だけ出力する
# ax.plot(x_th, y_th, color="#000000", linestyle='solid', linewidth=2, marker='o', markersize=0, label="theory")
ax.plot(x, y_lin, color="#000000", linestyle='solid', linewidth=2, marker='o', markersize=0, label=r"$x \propto t$") 

for i in range(1, 1840, 50):
    ax.plot(x[i], y[i], color="#000000", linestyle='solid', linewidth=2, marker='o', markersize=2)
