module constant
    ...
    integer, parameter :: step_fixed = Nstep           !円筒を止めておくステップ数
    ...
    real(8), parameter :: mu = 0.5d0, mu_cyliner = 0.5d0             !粒子, 円筒の密度
    ...
    real(8), parameter :: omega = -0.1d0
    !斜面のパラメータ
    real(8), parameter :: alpha_degree = 0.0d0
...
contains
    ...
    subroutine particle_contact_dissipation(d1, d2, x1, x2, v1, v2, w1, w2, x_center, dis, slip, ft)
        ...
        ! dis(nx,ny) = dis(nx,ny) - u * etan * (inner_product(vn(:),vn(:))) + slip*u*inner_product(ft(:),vt_rot(:))

        dis(nx,ny) = dis(nx,ny) - u * etan * (inner_product(vn(:),vn(:)))&
        - (1-slip) * u * etat * inner_product(v1(:)-v2(:),t(:))*inner_product(vt_rot(:),t(:))&
    ...
    subroutine particle_cylinder_contact_dissipation(x_center, v_center, w_wall, x, v, w, u_wall, Us, dis, slip_wall, fwt)
            ...
            dis(nx,ny) = dis(nx,ny) - u_wall * etan * (inner_product(vn(:),vn(:))) &
            - (1-slip_wall) * u_wall * etat * inner_product(v(:),t(:)) * inner_product(vt_rot(:),t(:)) &
            + slip_wall*u_wall*inner_product(fwt(:),vt_rot(:))
...
program dem_2d
    ...
    real(8) :: x_theory = 0
    real(8) :: theta_center(0:Nstep) = 0, w_wall(0:Nstep) = omega
    ...
    call makedirs('./dram_xy_data')
    call print_profile_condition

!初期状態を設定 {
    do step = 0, step_fixed
        x_center(0:1,step) = (/0.0d0, 0.0d0/)
    enddo
    ...
    do step = 1, Nstep 
        ...
        if (step > 500000 .and. mod(step, vtk_step)==0) then
            write(str_step, '(i7.7)') step
            open(40,file='./dram_xy_data/xy.'//str_step)
            do i = 1, particle_number
                write(40,*) x(:,i)
            enddo
            close(40)
    ...
    open(20,file='dissipation_ave_dram_'//d_char//'.d')
    do i = 0,v_cell-1
        do j = 0, v_cell - 1
            write(20,*) j, i, -dis_ave(j,i)    !単位時間あたりの散逸エネルギー
        enddo
    enddo


    write(*,*) sum(-dis_ave(:,:))   !系全体の単位時間あたりの散逸エネルギー
