...
contains
    ...
    subroutine mkvtk(step, dirname, x, theta)    !dirnameというフォルダ内にstep目のvtkファイルを作成
        ...
        do i = 1, particle_number
            write(10,'(f8.2,f8.2,f8.2)') x(:,i), 0.0d0
        enddo
        write(10,'(a)') ' '
        ...
        do i = 1, particle_number
            write(10,'(f6.2,f6.2,f6.2)') (d/2.0d0)*cos(theta(i)), (d/2.0d0)*sin(theta(i)), 0.0d0
        enddo
...
contains
    ...
    subroutine particle_contact_dissipation(d1, d2, x1, x2, v1, v2, w1, w2, x_center, dis, slip, ft, &
        x1_next, x2_next, v1_next, v2_next, w1_next, w2_next)
        integer, intent(in) :: slip
        real(8), intent(in) :: d1, d2
        real(8), intent(in) :: x1(0:1), x2(0:1), v1(0:1), v2(0:1)
        real(8), intent(in) :: w1, w2
        real(8), intent(in) :: x1_next(0:1), x2_next(0:1), v1_next(0:1), v2_next(0:1)
        real(8), intent(in) :: w1_next, w2_next
        real(8), intent(in) :: x_center(0:1)
        ...
        real(8) :: u = 0
        real(8) :: n(0:1) = 0, t(0:1) = 0
        real(8) :: n_next(0:1) = 0, t_next(0:1) = 0
        real(8) :: vn(0:1) = 0, vt_rot(0:1) = 0
        real(8) :: vn_next(0:1) = 0, vt_rot_next(0:1) = 0
        ...
        if (x1_next(0) /= x2_next(0) .or. x1_next(1) /= x2_next(1)) then 
            n_next(0:1) = (x1_next(0:1) - x2_next(0:1)) / vec_length(x1_next(0:1), x2_next(0:1))
        endif 
        ...
        vn(0:1) = inner_product(v1(0:1)-v2(0:1), n(0:1)) * n(0:1)     
        vn_next(0:1) = inner_product(v1_next(0:1)-v2_next(0:1), n_next(0:1)) * n_next(0:1) 
        vt_rot(0:1) = (inner_product(v1(0:1)-v2(0:1), t(0:1)) + (d1 * w1 + d2 * w2)/2) * t(0:1)
        vt_rot_next(0:1) = (inner_product(v1_next(0:1)-v2_next(0:1), t_next(0:1)) + &
        (d1 * w1_next + d2 * w2_next)/2) * t_next(0:1)
        nx=floor((x1(0)+x2(0))/2.0d0 - x_center(0))+v_cell/2
        ny=floor((x1(1)+x2(1))/2.0d0 - x_center(1))+v_cell/2
        dis(nx,ny) = dis(nx,ny) - (u * etan * (inner_product(vn(:),(vn(:)+vn_next(:))/2.0d0)) &
         + u*inner_product(ft(:),(vt_rot(:)+vt_rot_next(:))/2.0d0)) * step_width
    ...
    subroutine particle_cylinder_contact_dissipation(x_center, v_center, w_wall, x, v, w, u_wall, Us, dis, slip_wall, fwt, &
        x_center_next, v_center_next, w_wall_next, x_next, v_next, w_next)
            integer, intent(in) :: slip_wall
            real(8), intent(in) :: w_wall, w
            real(8), intent(in) :: x_center(0:1), v_center(0:1), x(0:1), v(0:1)
            real(8), intent(in) :: w_wall_next, w_next
            real(8), intent(in) :: x_center_next(0:1), v_center_next(0:1), x_next(0:1), v_next(0:1)
            real(8), intent(in) :: fwt(0:1)
            integer, intent(out) :: u_wall
            real(8), intent(inout) :: Us
            real(8), intent(inout) :: dis(0:v_cell-1,0:v_cell-1)
            real(8) :: vn(0:1) = 0, n(0:1) = 0, t(0:1) = 0, vt_rot(0:1) = 0
            real(8) :: vn_next(0:1) = 0, n_next(0:1) = 0, t_next(0:1) = 0, vt_rot_next(0:1) = 0
            integer :: nx = 0, ny = 0
            ...
            if (vec_length(x_center_next(0:1), x_next(0:1)) == 0) then
                n_next(0:1) = 0
            else 
                n_next(0:1) = (x_center_next(0:1) - x_next(0:1)) / vec_length(x_center_next(0:1), x_next(0:1))
            endif 
            ...
            vn(0:1) = inner_product(v(0:1) - v_center(0:1), n(0:1)) * n(0:1)
            vt_rot(0:1) = (inner_product(v(0:1) - v_center(0:1), t(0:1)) + (d / 2 * w - r_cylinder * w_wall)) * t(0:1)

            vn_next(0:1) = inner_product(v_next(0:1) - v_center_next(0:1), n_next(0:1)) * n_next(0:1)
            vt_rot_next(0:1) = (inner_product(v_next(0:1) - v_center_next(0:1), t_next(0:1)) +&
             (d / 2 * w_next - r_cylinder * w_wall_next)) * t_next(0:1)

            nx=floor(x(0) - x_center(0))+v_cell/2
            ny=floor(x(1) - x_center(1))+v_cell/2
            dis(nx,ny) = dis(nx,ny) - u_wall * etan * (inner_product(vn(:),(vn(:)+vn_next(:))/2.0d0)) &
            + slip_wall*u_wall*inner_product(fwt(:),(vt_rot(:)+vt_rot_next(:))/2.0d0)
...
program dem_2d
    ...
    real(8) :: n(0:1) = 0, t(0:1) = 0
    real(8) :: fcn(0:1) = 0, fct(0:1,1:particle_number,1:particle_number) = 0, fc_reaction(0:1,1:array_size) = 0
    ...
    integer :: counter = 0, slip_wall(1:particle_number) = 0
    integer :: slip(1:particle_number, 1:particle_number) = 0
    ...
    open(40,file='dissipation_t_2.d')
    ...
    do step = 1, Nstep 
        ...
        if(particle_number > 0) then 
            ...
            do i = 1, particle_number   !注目する粒子
                        ...
                        do 
                            ...
                            elseif (tmp /= i) then
                                if(i < tmp) then
                                    ...
                                    fcn(0:1), fct(0:1,i,tmp), torque, Us, slip(i,tmp))
                                    fc_reaction(0:1,tmp) = fc_reaction(0:1,tmp) - fcn(0:1) - fct(0:1,i,tmp)
                                    torque_reaction(tmp) = torque_reaction(tmp) + torque
                                    Uall = Uall + Us
                                endif

                                fc_all(0:1,i) = fc_all(0:1,i) + fcn(0:1) + fct(0:1,i,tmp)
                ...
                x(0:1,i) = x_prev(0:1,i) + ((v(0:1,i)+v_prev(0:1,i))/2) * step_width
                        ...
                        do 
                            ...
                            elseif (tmp /= i) then
                                if(i < tmp) then
                                    ...
                                    call particle_contact_dissipation(d, d, x_prev(0:1,i), x_prev(0:1,tmp) + x_tmp(0:1), &
                                    v_prev(0:1,i), v_prev(0:1,tmp), w_prev(i), w_prev(tmp), x_center(0:1,step-1), dis(:,:), &
                                    slip(i,j), fct(0:1,i,j), x(:,i), x(:,tmp), v(:,i), v(:,tmp), w(i), w(tmp))
        ...
        dis_ave(:,:) = dis_ave(:,:) + dis(:,:)
        write(40,*) step, sum(-dis(:,:)), Uall, sum(-dis_ave(:,:)), Uall+sum(-dis_ave(:,:))
        Uall = 0.0d0
        dis(:,:) = 0.0d0
    ...
    open(20,file='dissipation_ave_'//d_char//'.d')
    do i = 0,v_cell-1
        do j = 0, v_cell - 1
            write(20,*) j, i, -dis_ave(j,i)
        enddo
    enddo

    open(30,file='vt_data')
    do step = 0, Nstep
    ...
    write(*,*) counter
    write(*,*) sum(-dis_ave(:,:))
    write(*,*) (m_cylinder+particle_number*m)*sin(alpha)*sum(v_center(0,200000:500000))/300001
