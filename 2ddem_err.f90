module constant
    ...
    integer, parameter :: sample = 5, step_sample = 50000
    integer, parameter :: Nstep = 1000000, particle_number = 10, time_step = 1000      !time_stepは描写する時間間隔
    integer, parameter :: graph_step = 500, vtk_step = 500
...
program dem_2d
    ...
    integer :: i = 0, j = 0, step = 0, u = 0, round = 0, progress = 0, smp = 0
    ...
    real(8) :: x_center(0:1,0:Nstep) = 0, v_center(0:1,0:Nstep) = 0, p_center(0:1,0:Nstep) = 0
    real(8) :: x_center_tmp(0:1) = 0, x_contact(0:1) = 0, x_contact_rot(0:1) = 0
    real(8) :: x_center_sample(0:Nstep, 1:sample) = 0.0d0
    real(8) :: x_center_err(0:Nstep, 1:2) = 0.0d0
    ...
    character(len = *), parameter :: base_xt = "./xt_err_", base_vt = "./vt_data/vt_n=", base_fr = './fr_'
    ...
    open(20,file = 'xt_cylinder_all.d')
    ...
    do smp = 1, sample
        ...
        do step = 1, Nstep 
            ...
            !円筒の変位の時間変化
            if (step >= step_fixed) then
                x_center_sample(step - step_fixed,smp) = x_original(0,step) -x_center(0,0)
            endif
        ...
        enddo
        progress = 0
        print *, 'sample',smp,'complete'
    enddo

    do step = 0, Nstep - step_fixed, step_sample
        x_center_err(step,1) = sum(x_center_sample(step,:)) / sample
        x_center_err(step,2) = sqrt(sum((x_center_sample(step,:)-x_center_err(step,1))**2)/sample)/sqrt(dble(sample))

        write(15,*) step * step_width, x_center_err(step,:) / (2.0d0*Pi*(r_cylinder+deltad))
        write(20,*) x_center_sample(step,1:3) / (2.0d0*Pi*(r_cylinder+deltad))
    enddo
    close(15)
    close(20)
